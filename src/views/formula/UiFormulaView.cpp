...
public:
	...
	void append(Render::FrameData& fd) const override {
		...
		fd.roundedRects.push_back(Render::RoundedRectCmd{ .rect = QRectF(m_viewport), .radiusPx = 0.0f, .color = m_color, .clipRect = QRectF(m_viewport) });
	}
...
public:
	...
	void updateResourceContext(IconLoader& loader, QOpenGLFunctions* gl, float dpr) override { if (m_child) m_child->updateResourceContext(loader, gl, dpr); }
