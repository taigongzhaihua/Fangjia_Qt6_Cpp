...
{
public:
	explicit VSplitter(QColor color, int logicalW = 1)
		: m_color(color), m_w(std::max(1, logicalW))
	{
	}
	...
	// 给父布局的首选尺寸：仅提供宽度，交叉轴高度交给父布局 Stretch
	QRect bounds() const override { return QRect(0, 0, m_w, 0); }
				...
				auto section = [&](const QString& head, const QString& content) -> WidgetPtr
					{
						...
						return panel({
							...
							->orientation(UiPanel::Orientation::Vertical)
							->spacing(2);
					};
