...
public:
	...
	WidgetPtr buildUI() {
		...
		auto mainColumn = UI::make_widget<Column>(WidgetList{
			// 欢迎标题
			...
			UI::make_widget<Container>(
				...
				// 底部空间
				make_widget<Expanded>(make_widget<Spacer>())
			});

		// 设置主列的属性
		mainColumn->mainAxisAlignment(Alignment::Start);
...
private:
	WidgetPtr buildFeatureGrid() {
		return make_widget<Container>(
			make_widget<Column>(WidgetList{
				make_widget<Row>(WidgetList{
					buildFeatureCard(
	...
	WidgetPtr buildFeatureCard(const QString& iconPath, const QString& title, const QString& desc) {
		...
		auto column = UI::make_widget<Column>(WidgetList{
			icon,
		...
		column->mainAxisAlignment(Alignment::Center);
