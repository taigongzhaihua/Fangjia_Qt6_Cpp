...
void UiScrollView::updateChildLayout() const
{
	if (!m_child) return;
...
QRect UiScrollView::getScrollbarThumbRect() const {
	...
	const int thumbHeight = std::max(THUMB_MIN_HEIGHT,
		static_cast<int>(static_cast<float>(scrollbarRect.height()) * ratio));
	...
	const int thumbTop = scrollbarRect.top() + static_cast<int>(static_cast<float>(availableTrackHeight) * scrollRatio);

	return { scrollbarRect.left(), thumbTop, scrollbarRect.width(), thumbHeight };
}
...
void UiScrollView::handleTrackClick(const QPoint& pos) {
	...
	const int targetScrollY = static_cast<int>(static_cast<float>(maxScrollY()) * scrollRatio);
