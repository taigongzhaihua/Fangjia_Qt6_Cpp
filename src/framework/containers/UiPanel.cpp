...
void UiPanel::addChild(IUiComponent* c, CrossAlign a)
{
    if (!c) return;
    for (const auto& ch : m_children) if (ch.component == c) return;
    m_children.push_back(Child{ c, a, true });
...
QSize UiPanel::measureChild(IUiComponent* c, const int crossAvail) const
{
    ...
    if (auto* l = dynamic_cast<ILayoutable*>(c)) {
        ...
        if (m_orient == Orientation::Horizontal) {
            return l->measure(SizeConstraints{ 0,0, std::numeric_limits<int>::max()/2, std::max(0, crossAvail) });
        } else {
            return l->measure(SizeConstraints{ 0,0, std::max(0, crossAvail), std::numeric_limits<int>::max()/2 });
        }
    }
    ...
    // 交叉轴限制
    if (m_orient == Orientation::Horizontal) {
        return QSize(std::max(0, s.width()), std::min(std::max(0, crossAvail), std::max(0, s.height())));
    } else {
        return QSize(std::min(std::max(0, crossAvail), std::max(0, s.width())), std::max(0, s.height()));
    }
}
...
QRect UiPanel::placeChild(const QRect& area, const int cur, const QSize& desired, const CrossAlign a) const
{
    if (m_orient == Orientation::Horizontal) {
        ...
        int y = area.top();
        switch (a) {
        case CrossAlign::Center: y = area.center().y() - h/2; break;
        case CrossAlign::End:    y = area.bottom() - h; break;
        case CrossAlign::Stretch: default: y = area.top(); break;
        }
        return QRect(area.left() + cur, y, w, (a == CrossAlign::Stretch ? area.height() : h));
    } else {
        const int h = std::max(0, desired.height());
        const int w = std::max(0, std::min(desired.width(), area.width()));
        int x = area.left();
        switch (a) {
        case CrossAlign::Center: x = area.center().x() - w/2; break;
        case CrossAlign::End:    x = area.right() - w; break;
        case CrossAlign::Stretch: default: x = area.left(); break;
        }
        return QRect(x, area.top() + cur, (a == CrossAlign::Stretch ? area.width() : w), h);
    }
}
...
void UiPanel::updateLayout(const QSize& windowSize)
{
    ...
    for (size_t i = 0; i < m_children.size(); ++i) {
        ...
        const QRect r = placeChild(area, cur, desired[i], ch.crossAlign);
        m_childRects[i] = r;
...
void UiPanel::updateResourceContext(IconLoader& loader, QOpenGLFunctions* gl, float devicePixelRatio)
{
    m_loader = &loader; m_gl = gl; m_dpr = std::max(0.5f, devicePixelRatio);
    for (auto& ch : m_children) if (ch.component) ch.component->updateResourceContext(loader, gl, devicePixelRatio);
}
...
bool UiPanel::onMousePress(const QPoint& pos)
{
    ...
    for (auto it = m_children.rbegin(); it != m_children.rend(); ++it) {
        if (it->visible && it->component && it->component->onMousePress(pos)) { m_capture = it->component; return true; }
    }
    return false;
}
...
{
    if (m_capture) { bool h = m_capture->onMouseRelease(pos); m_capture = nullptr; return h; }
    for (auto it = m_children.rbegin(); it != m_children.rend(); ++it)
...
{
    bool any = false;
    for (auto& ch : m_children) if (ch.component) any = ch.component->tick() || any;
    return any;
}

void UiPanel::onThemeChanged(bool isDark)
{
    for (auto& ch : m_children) if (ch.component) ch.component->onThemeChanged(isDark);
}