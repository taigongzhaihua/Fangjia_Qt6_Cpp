...
void UiGrid::addChild(IUiComponent* c, int row, int col, int rowSpan, int colSpan,
    ...
    m_children.push_back(Child{ c, row, col, rowSpan, colSpan, hAlign, vAlign, true });
...
std::vector<int> UiGrid::computeColumnWidths(const int contentW) const {
    ...
    for (const auto& ch : m_children) {
        ...
        for (int c = c0; c <= c1; ++c) {
            ...
            if (def.type == TrackDef::Type::Pixel) sum += width[c];
            else if (def.type == TrackDef::Type::Auto) { sum += width[c]; autoCount++; }
            else { sum += starMin[c]; sumStarW += starWeight[c]; starCount++; }
        }
        // 加上列内间距
        sum += std::max(0, ch.colSpan - 1) * m_colSpacing;

        int need = nat.width() - sum;
        if (need <= 0) continue;
...
std::vector<int> UiGrid::computeRowHeights(const int contentH, const std::vector<int>& colW) const {
    ...
    auto spanWidth = [&](int col, int colSpan) -> int {
...
void UiGrid::updateLayout(const QSize& windowSize) {
    ...
    auto spanSize = [&](int start, int span, const std::vector<int>& arr, int gap) {
...
void UiGrid::updateResourceContext(IconLoader& loader, QOpenGLFunctions* gl, float devicePixelRatio) {
...
void UiGrid::onThemeChanged(bool isDark) {
